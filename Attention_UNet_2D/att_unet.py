...
def crop_tensor(encoder_tensor, decoder_tensor):
    '''
    Crop encoder tensor to match with decoder tensor size.
...
class AttentionBlock(nn.Module):
    def __init__(self, f_x: int, f_g: int, f_int: int, att_activation: str='relu', att_norm=None):
        '''
        Input
        -------
        f_x : No. of filters in x (i.e. key and value)
        f_g: No. of filters in g (i.e. query/gate)
        f_int: No. of intermediate filters
        att_activation: Activation function for attention
        att_norm: Normalization for attention. If 'None', then no normalization.
        ...
        self.att_activation = activations(att_activation)
        if att_norm == None:
            self.theta_x = nn.Conv2d(f_x, f_int, kernel_size=1, stride=1, bias=True)
            self.phi_g = nn.Conv2d(f_g, f_int, kernel_size=1, stride=1, bias=True)
            self.psi = nn.Conv2d(f_int, 1, kernel_size=1, stride=1, bias=True)
        else:
            layers_x = [nn.Conv2d(f_x, f_int, kernel_size=1, stride=1, bias=True),
                      normalization(att_norm, f_int)]
            self.theta_x = nn.Sequential(*layers_x)
            
            layers_g = [nn.Conv2d(f_g, f_int, kernel_size=1, stride=1, bias=True),
                      normalization(att_norm, f_int)]
            self.phi_g = nn.Sequential(*layers_g)
            
            layers_psi = [nn.Conv2d(f_int, 1, kernel_size=1, stride=1, bias=True),
                          normalization(att_norm, 1),
                          nn.Sigmoid()]
    ...
    def forward(self, g, x):
        ...
        phi_g = self.phi_g(g)
        psi = self.att_activation(theta_x + phi_g)
        psi = self.psi(psi)
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in reversed(range(len(for_concat))): # e.g. i = 3,2,1,0
            ...
            # Apply attention
            f_x, f_g, f_int = cropped_encoder.size(1), x.size(1), x.size(1) // 2
            self.attention = AttentionBlock(f_x, f_g, f_int, self.att_activation, self.att_norm)
            attention = self.attention(g=x, x=cropped_encoder) # don't get confused. In g=x, x is basically gate, not key.
            # Concatenate
